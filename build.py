...
def _filter_strip_md(value: str) -> Markup:
    """
    Converts **bold** Markdown to <b> tags (safe for WeasyPrint HTML rendering)
    and strips italic and inline-code markers so raw YAML fields like
    '*italic*' or '`code`' don't leak literal punctuation into the PDF.

    Handles:
      **bold**  →  <b>bold</b>
      __bold__  →  <b>bold</b>
      *italic*  →  italic        (markers stripped, not rendered)
      _italic_  →  italic        (markers stripped, not rendered)
      `code`    →  code          (markers stripped, not rendered)
    """
    if not isinstance(value, str):
        return Markup(str(value))
    # Bold: convert to <b> tags (WeasyPrint renders these natively)
    value = re.sub(r"\*\*(.+?)\*\*", r"<b>\1</b>", value)
    value = re.sub(r"__(.+?)__", r"<b>\1</b>", value)
    # Italic: strip markers only (single * or _ not already consumed above)
...
def _filter_render_md_web(value: str) -> Markup:
    """
    Converts **bold** Markdown to <strong> tags for the web HTML template,
    and strips italic/code markers. Returns a Markup object so Jinja2
    does not double-escape the injected HTML tags.
    """
    if not isinstance(value, str):
        return Markup(str(value))
    # Bold: convert to <strong> (semantic HTML for web)
...
def compile_site(
    ...
    The format_date filter is registered here so the web template renders human-readable
    dates (e.g. "Jun 2025") rather than raw ISO-8601 strings ("2025-06-20").
    """
    try:
        env = _make_jinja_env(
            template_dir,
            extra_filters={
                ...
                "render_md": _filter_render_md_web,
            },
        )
...
def compile_pdf(data: dict, pdf_css: str, template_dir: Path, output_file: Path) -> None:
    ...
    try:
        env = _make_jinja_env(
            ...
            extra_filters={
                "format_date": _filter_format_date,
                "strip_md": _filter_strip_md,   # now renders **bold** as <b> tags
            },
        )
...
def main():
    ...
    # format_date is shared so the web template renders human-readable dates
    # (e.g. "Jun 2025") instead of raw ISO strings ("2025-06-20").
    compile_site(
        data=resume_data,
